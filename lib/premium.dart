...
class _PremiumScreenState extends State<PremiumScreen>
  ...
  static const Color _accentBorder  = Color(0x600077B6);
  static const Color _accentTint    = Color(0xFFD6EEFF);
  ...
  static const Color _fieldBg       = Color(0xFFE8F4FC);
  static const Color _card          = Color(0x1A0077B6);

  static const Color _gold          = Color(0xFFF5A623);
  static const Color _goldTint      = Color(0xFFFFF3E0);
  static const Color _goldBorder    = Color(0x60F5A623);
  static const Color _green         = Color(0xFF27AE60);
  static const Color _greenTint     = Color(0xFFE8F8EF);
  static const Color _greenBorder   = Color(0x6027AE60);

  static const Color _cardBlueBg    = Color(0xFFE0F0FB);
  static const Color _cardGoldBg    = Color(0xFFFFF8E8);
  static const Color _cardGreenBg   = Color(0xFFEAF8F0);
  ...
  // Geometry colors matching LandingPage
  static const Color _geoAccent = Color(0xFF1A8CB8);
  static const Color _geoGreen  = Color(0xFF1D9E75);
  ...
  // ── Navigate to AppShell (main.dart) ──────────────────────────────────────
  ...
  Future<void> handleGoogleSignIn() async {
    ...
    try {
      if (kIsWeb) { await _handleGoogleSignInWeb(); } else { await _handleGoogleSignInMobile(); }
    } finally {
  ...
  Future<void> _handleGoogleSignInWeb() async {
    try {
      ...
      if (mounted) {
        ...
        if (isNewUser) await _showSuccessDialog(name: displayName, isGoogle: true);
        widget.onLogin();
        _navigateToAppShell();
      }
    } on FirebaseAuthException catch (e) {
      ...
      if (errStr.contains('popup') || errStr.contains('cancel') || errStr.contains('closed')) return;
  ...
  Future<void> _handleGoogleSignInMobile() async {
    try {
      ...
      if (mounted) {
        final isNewUser = userCredential.additionalUserInfo?.isNewUser ?? false;
        if (isNewUser) await _showSuccessDialog(name: googleUser.displayName ?? 'there', isGoogle: true);
        widget.onLogin();
        _navigateToAppShell();
      }
    } on FirebaseAuthException catch (e) {
      ...
      builder: (context) => Dialog(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: _accent.withOpacity(0.15), blurRadius: 30, spreadRadius: 2)],
          ),
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const Text("Account Created!", style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: _textDark, letterSpacing: -0.3)),
            const SizedBox(height: 8),
            Text(
              isGoogle ? "Welcome, $name!\nSigned in with Google successfully." : "Welcome to CleftTune.\nYou're all set to get started!",
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: _accent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
  ...
  // ── Geometric box shapes — mirrors LandingPage._buildBoxShapes() ──────────
  Widget _buildBoxShapes() {
    final f = _orbAnim.value;
    return Stack(
      children: [
        // ── Top-right cluster ──
        _box(top: 28  - 10 * f, right: 235, size: 50,  rotate: 0.49,  color: _geoGreen.withOpacity(0.07)),
        _box(top: 78  -  6 * f, right: 172, size: 33,  rotate: -0.31, color: _geoAccent.withOpacity(0.07)),
        _box(top: 10  +  8 * f, right: 118, size: 64,  rotate: 0.73,  color: _geoGreen.withOpacity(0.06)),
        _box(top: 68  +  5 * f, right: 68,  size: 37,  rotate: -0.52, color: _geoAccent.withOpacity(0.08)),
        _box(top: 18  -  4 * f, right: 22,  size: 27,  rotate: 0.96,  color: _geoGreen.withOpacity(0.07)),
        _box(top: 105 +  6 * f, right: 190, size: 22,  rotate: 1.10,  color: _geoAccent.withOpacity(0.06), outlineOnly: true),
        _box(top: 55  -  3 * f, right: 48,  size: 18,  rotate: -0.70, color: _geoGreen.withOpacity(0.09), outlineOnly: true),
        _box(top: 130 -  7 * f, right: 10,  size: 42,  rotate: 0.30,  color: _geoAccent.withOpacity(0.05)),

        // ── Bottom-right cluster ──
        _box(bottom: 98  +  8 * f, right: 205, size: 46, rotate: -0.38, color: _geoAccent.withOpacity(0.06)),
        _box(bottom: 48  -  6 * f, right: 132, size: 58, rotate: 0.63,  color: _geoGreen.withOpacity(0.07)),
        _box(bottom: 88  +  4 * f, right: 58,  size: 31, rotate: -0.84, color: _geoAccent.withOpacity(0.07)),
        _box(bottom: 140 -  5 * f, right: 12,  size: 24, rotate: 0.55,  color: _geoGreen.withOpacity(0.08), outlineOnly: true),
        _box(bottom: 30  +  9 * f, right: 82,  size: 19, rotate: -1.20, color: _geoAccent.withOpacity(0.07), outlineOnly: true),
        _box(bottom: 60  -  4 * f, right: 168, size: 28, rotate: 0.80,  color: _geoGreen.withOpacity(0.06)),
        _box(bottom: 175 +  3 * f, right: 240, size: 20, rotate: -0.45, color: _geoAccent.withOpacity(0.06), outlineOnly: true),

        // ── Left-edge shapes ──
        _box(top: 138  + 10 * f, left: 8,   size: 30, rotate: 0.40,  color: _geoAccent.withOpacity(0.05)),
        _box(bottom: 158 -  8 * f, left: 18, size: 40, rotate: -0.60, color: _geoGreen.withOpacity(0.05)),
        _box(top: 60   -  5 * f, left: 30,  size: 20, rotate: 1.00,  color: _geoGreen.withOpacity(0.06), outlineOnly: true),
        _box(top: 200  +  6 * f, left: 5,   size: 16, rotate: -0.80, color: _geoAccent.withOpacity(0.06), outlineOnly: true),
        _box(bottom: 80  +  4 * f, left: 40, size: 24, rotate: 0.65,  color: _geoAccent.withOpacity(0.05)),

        // ── Top-left corner ──
        _box(top: 20   +  4 * f, left: 60,  size: 34, rotate: -0.35, color: _geoGreen.withOpacity(0.05)),
        _box(top: 80   -  6 * f, left: 100, size: 18, rotate: 0.90,  color: _geoAccent.withOpacity(0.05), outlineOnly: true),

        // ── Bottom-left corner ──
        _box(bottom: 30  +  5 * f, left: 65,  size: 26, rotate: 0.50,  color: _geoGreen.withOpacity(0.06), outlineOnly: true),
        _box(bottom: 90  -  4 * f, left: 110, size: 16, rotate: -1.00, color: _geoAccent.withOpacity(0.06)),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        fit: StackFit.expand,
        children: [
          // 1. Gradient background (matching LandingPage)
          Container(
          ...
          // 2. Animated geometric box shapes
          AnimatedBuilder(
            animation: _orbAnim,
  ...
  Widget _buildDesktop() => Row(
        ...
        children: [
          ...
          Container(width: 1, color: _accent.withOpacity(0.15)),
          Expanded(
            child: Center(
              child: SizedBox(
                width: 380,
          ...
          Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: _accent.withOpacity(0.35), blurRadius: 10, offset: const Offset(0, 4))],
              ),
            ...
            RichText(
              text: const TextSpan(children: [
                TextSpan(text: 'Cleft', style: TextStyle(color: _textDark, fontWeight: FontWeight.bold, fontSize: 20, letterSpacing: -0.5)),
                TextSpan(text: 'Tune', style: TextStyle(color: _accent, fontWeight: FontWeight.bold, fontSize: 20, letterSpacing: -0.5)),
              ]),
            ),
          ]),
          const SizedBox(height: 12),
          const Text('Your voice, trained by you.',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: _textDark, height: 1.2, letterSpacing: -0.3)),
          const SizedBox(height: 6),
          const Text('A real-time speech assistant for people with cleft palate. It learns your unique voice and improves every session.',
              ...
              builder: (context, constraints) {
                return Column(
                  children: [
                    Row(
                      children: [
                        _squareInfoCard(icon: Icons.mic_rounded,            title: 'Speak',   subtitle: 'Capture speech in real time via mic.',      color: _accent, bgColor: _cardBlueBg, flex: 1),
                        const SizedBox(width: 10),
                        _squareInfoCard(icon: Icons.auto_fix_high_rounded,  title: 'Correct', subtitle: 'Teach AI when it mishears you.',              color: _green,  bgColor: _cardGreenBg, flex: 1),
                        const SizedBox(width: 10),
                        _squareInfoCard(icon: Icons.trending_up_rounded,    title: 'Improve', subtitle: 'AI learns your patterns every session.',      color: _gold,   bgColor: _cardGoldBg, flex: 1),
                      ],
                    ),
                          ...
                          child: AspectRatio(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                boxShadow: [BoxShadow(color: _accent.withOpacity(0.3), blurRadius: 12, spreadRadius: 1)],
                              ),
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Container(
                                  width: 38, height: 38,
                                ...
                                const Spacer(),
                                const Text('Daily Streak', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700)),
                                const SizedBox(height: 3),
                                const Text('Train daily to keep it alive.', style: TextStyle(color: Colors.white70, fontSize: 10, height: 1.3)),
                              ]),
                            ),
                          ),
                        ),
                          ...
                          child: AspectRatio(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                boxShadow: [BoxShadow(color: _accent.withOpacity(0.10), blurRadius: 8, spreadRadius: 0)],
                              ),
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Container(
                                  width: 32, height: 32,
                                  decoration: BoxDecoration(color: _cardBlueBg, borderRadius: BorderRadius.circular(8)),
                                  child: const Icon(Icons.task_alt_rounded, color: _accent, size: 16),
                                ),
                                const Spacer(),
                                const Text('Daily Tasks', style: TextStyle(color: _textDark, fontSize: 12, fontWeight: FontWeight.w700)),
                                const SizedBox(height: 6),
                                _miniTaskRow(Icons.spellcheck_rounded,        '5 corrections', _accent),
                                const SizedBox(height: 4),
                                _miniTaskRow(Icons.record_voice_over_rounded, '3 sentences',   _green),
                                const SizedBox(height: 4),
                                _miniTaskRow(Icons.library_add_rounded,       '10 words',      _gold),
                              ]),
                            ),
                          ),
                        ),
                          ...
                          child: AspectRatio(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                boxShadow: [BoxShadow(color: _green.withOpacity(0.10), blurRadius: 8, spreadRadius: 0)],
                              ),
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Container(
                                  width: 32, height: 32,
                                  decoration: BoxDecoration(color: _cardGreenBg, borderRadius: BorderRadius.circular(8)),
                                  child: const Icon(Icons.shield_outlined, color: _green, size: 16),
                                ),
                                const Spacer(),
                                const Text('Private & Safe', style: TextStyle(color: _textDark, fontSize: 12, fontWeight: FontWeight.w700)),
                                const SizedBox(height: 4),
                                const Text('Speech data is private and tied only to your account.',
                                    style: TextStyle(color: _green, fontSize: 10, height: 1.4, fontWeight: FontWeight.w500)),
                                const SizedBox(height: 8),
                                Row(children: [
                                  _feedbackStar(true), _feedbackStar(true), _feedbackStar(true), _feedbackStar(true), _feedbackStar(false),
                                ]),
                              ]),
                            ),
                          ),
        ...
        child: AspectRatio(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: color.withOpacity(0.12), blurRadius: 8, spreadRadius: 0)],
            ),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                width: 34, height: 34,
              ...
              const Spacer(),
              Text(title, style: const TextStyle(color: _textDark, fontSize: 12, fontWeight: FontWeight.w700)),
              const SizedBox(height: 3),
              Text(subtitle, style: const TextStyle(color: _textSub, fontSize: 10, height: 1.3)),
            ]),
          ),
        ),
      );
  ...
  Widget _buildLoginCard() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: _accentBorder),
        boxShadow: [BoxShadow(color: _accent.withOpacity(0.10), blurRadius: 20, spreadRadius: 0)],
      ),
      child: Column(children: [
        Container(
          width: 52, height: 52,
          decoration: BoxDecoration(color: _cardBlueBg, borderRadius: BorderRadius.circular(14), border: Border.all(color: _accentBorder)),
        ...
        SizedBox(
          ...
          child: OutlinedButton(
            style: OutlinedButton.styleFrom(
              ...
              backgroundColor: _cardBlueBg,
            ),
            onPressed: isGoogleLoading ? null : handleGoogleSignIn,
            child: isGoogleLoading
                ? SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: _accent, strokeWidth: 2))
        ...
        Row(children: [
          Expanded(child: Divider(color: _accent.withOpacity(0.2))),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 12),
            child: Text("or use email", style: const TextStyle(fontSize: 11, color: _textSub)),
          ),
          Expanded(child: Divider(color: _accent.withOpacity(0.2))),
        ]),
        const SizedBox(height: 16),
        ...
        const SizedBox(height: 6),
        _buildTextField(controller: emailController, hint: "you@example.com", icon: Icons.email_outlined, keyboardType: TextInputType.emailAddress),
        const SizedBox(height: 14),
        ...
        _buildTextField(
          ...
          trailing: IconButton(
            icon: Icon(obscurePassword ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: _textSub, size: 18),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.shield_outlined, size: 13, color: _textSub),
          const SizedBox(width: 5),
          const Text("Your data is safe and private.", style: TextStyle(fontSize: 11, color: _textSub)),
        ]),
      ]),
    );
  }
  ...
  Widget _buildPasswordStrengthBar() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
            ...
            children: List.generate(4, (index) {
              ...
              return Expanded(
                child: Container(
                  ...
                  height: 4,
                  decoration: BoxDecoration(
                    color: filled ? _strengthColor : _accent.withOpacity(0.15),
        ...
        const SizedBox(width: 10),
        Text(_passwordStrengthLabel, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: _strengthColor)),
      ]),
      const SizedBox(height: 8),
      _buildPasswordHints(),
    ]);
  ...
  Widget _buildPasswordHints() {
    ...
    return Wrap(
      ...
      children: hints.map((h) => Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(h.$1 ? Icons.check_circle_rounded : Icons.circle_outlined, size: 12, color: h.$1 ? _accent : _white30),
  ...
  Widget _buildGoogleLogo() => SizedBox(width: 20, height: 20, child: CustomPaint(painter: _GoogleLogoPainter()));

  Widget _buildFieldLabel(String label) => Align(
        alignment: Alignment.centerLeft,
        child: Text(label, style: const TextStyle(fontSize: 11, color: _label, fontWeight: FontWeight.w600, letterSpacing: 0.3)),
      );
    ...
    TextInputType? keyboardType,
      ...
      TextField(
        ...
        decoration: InputDecoration(
          ...
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _accent.withOpacity(0.2))),
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _accent.withOpacity(0.2))),
...
class _ForgotPasswordScreenState extends State<_ForgotPasswordScreen>
  ...
  static const _accent       = Color(0xFF0077B6);
  static const _accentTint   = Color(0xFFD6EEFF);
  static const _accentBorder = Color(0x600077B6);
  static const _textDark     = Color(0xFF0D2B4E);
  static const _textSub      = Color(0xFF5A7A96);
  static const _fieldBg      = Color(0xFFE8F4FC);
  static const Color _geoAccent = Color(0xFF1A8CB8);
  static const Color _geoGreen  = Color(0xFF1D9E75);
  ...
  bool _isValidEmail(String e) => RegExp(r'^[\w\.-]+@[\w\.-]+\.\w{2,}$').hasMatch(e);

  Future<void> _sendReset() async {
    final email = _emailCtrl.text.trim();
    if (email.isEmpty) { setState(() => _errorText = "Please enter your email address."); return; }
    if (!_isValidEmail(email)) { setState(() => _errorText = "Enter a valid email address."); return; }
    setState(() { _errorText = null; _stage = 'loading'; });
    final connected = (await Connectivity().checkConnectivity()).any((r) => r != ConnectivityResult.none);
    if (!connected) {
  ...
  void _startCooldown() {
    ...
    _cooldownTimer = Timer.periodic(const Duration(seconds: 1), (t) {
      ...
      if (_cooldownSecs <= 0) { t.cancel(); if (mounted) setState(() => _resendCooldown = false); }
    });
  }
  ...
  Widget _buildInputState() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: _accentBorder),
        boxShadow: [BoxShadow(color: _accent.withOpacity(0.10), blurRadius: 20)],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Center(
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: const Color(0xFF91D8FF).withOpacity(0.5), blurRadius: 24, spreadRadius: 2)],
            ),
            child: const Icon(Icons.lock_reset_rounded, color: _accent, size: 36),
          ),
        ),
        const SizedBox(height: 28),
        const Center(child: Text("Reset your password",
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12),
                borderSide: BorderSide(color: _errorText != null ? Colors.redAccent.withOpacity(0.6) : _accent.withOpacity(0.2))),
          ...
          child: ElevatedButton(
            ...
            child: _stage == 'loading'
                ...
                : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    SizedBox(width: 10),
                    Text("Send Reset Link", style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 0.2)),
                  ]),
          ),
        ),
        const SizedBox(height: 20),
        Center(
          child: GestureDetector(
            ...
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              SizedBox(width: 5),
              Text("Back to Sign In", style: TextStyle(fontSize: 13, color: _textSub, fontWeight: FontWeight.w500)),
            ]),
          ),
        ),
        const SizedBox(height: 24),
  ...
  Widget _buildSuccessState() {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 32),
        const Text("Check your inbox!", style: TextStyle(fontSize: 24, fontWeight: FontWeight.w800, color: _textDark, letterSpacing: -0.5)),
        const SizedBox(height: 12),
        const Text("We've sent a password reset link to", textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: _textSub)),
        const SizedBox(height: 6),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          decoration: BoxDecoration(color: _accentTint, borderRadius: BorderRadius.circular(30), border: Border.all(color: _accentBorder)),
        ...
        Container(
          ...
          child: const Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Icon(Icons.warning_amber_rounded, color: Color(0xFFE67E22), size: 16),
            SizedBox(width: 10),
            Expanded(child: Text(
          ...
          child: OutlinedButton(
            ...
            child: _resendCooldown
                ? Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    const SizedBox(width: 8),
                    Text("Resend in ${_cooldownSecs}s", style: const TextStyle(fontSize: 14, color: _textSub, fontWeight: FontWeight.w500)),
                  ])
                : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.refresh_rounded, color: _accent, size: 16),
                    SizedBox(width: 8),
                    Text("Resend Email", style: TextStyle(fontSize: 14, color: _accent, fontWeight: FontWeight.w600)),
                  ]),
          ),
        ),
        const SizedBox(height: 16),
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(width: 8),
              Text("Back to Sign In", style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white)),
            ]),
          ),
        ),
        const SizedBox(height: 24),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...steps.asMap().entries.map((entry) {
          final i = entry.key; final step = entry.value; final isLast = i == steps.length - 1;
          return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Column(children: [
              Container(
                width: 32, height: 32,
                decoration: BoxDecoration(color: _accentTint, shape: BoxShape.circle, border: Border.all(color: _accentBorder)),
                child: Icon(step.$1, color: _accent, size: 15),
              ),
              if (!isLast) Container(width: 1.5, height: 24, color: _accent.withOpacity(0.2)),
            ]),
            const SizedBox(width: 14),
            Expanded(child: Padding(
              padding: const EdgeInsets.only(top: 6),
              child: Text(step.$2, style: const TextStyle(fontSize: 13, color: _textSub, height: 1.5)),
            )),
          ]);
        }),
      ]),
...
class _GoogleLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      canvas.drawArc(Rect.fromCircle(center: c, radius: r), angle * (3.14159 / 180), 90 * (3.14159 / 180), true, paint);
      angle += 90;
    }
